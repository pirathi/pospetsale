...
class HomeController extends Controller
{
    ...
    public function getProductStockAlert()
    {
        if (request()->ajax()) {
            ...
            return Datatables::of($products)
                ...
                ->editColumn('lastMatedDate', function ($row) {
                    $lmd = $row->product_custom_field7;
                    return $lmd;
                })
                ->addColumn('lmd55d', function ($row) {
                    $lmd = $row->product_custom_field7;
                    $lmd55d = date('Y-m-d', strtotime($lmd. ' + 55 days'));
                    return $lmd55d;
                })
                ->addColumn('lmd5mo', function ($row) {
                    $lmd5 = $row->product_custom_field7;
                    $lmd55m = date('Y-m-d', strtotime($lmd5. ' + 165 days'));
                    return $lmd55m;
                })
    ...
    //puppy remainder
    public function getPuppiesDetails()
    {
        if (request()->ajax()) {
            $business_id = request()->session()->get('user.business_id');
            ...
            return Datatables::of($products)
                ...
                ->addColumn('product', function ($row) {
                    if ($row->type == 'variable') {
                        return $row->puppy_name . ' (' . $row->sub_sku . ')';
                    } 
                })
                ->editColumn('Date of Birth', function ($row) {
                    $dob = $row->puppy_dob;
                    return $dob;
                })
                ->addColumn('Worming', function ($row) {
                    $dob = $row->puppy_dob;
                    $warming = date('Y-m-d', strtotime($dob. ' + 14 days'));
                    return $warming;
                })
                ->addColumn('lmd5mo', function ($row) {
                    $lmd5 = $row->product_custom_field7;
    ...
    //puppy vacination
    public function getPuppiesVaccination()
    {
        if (request()->ajax()) {
            $business_id = request()->session()->get('user.business_id');
            ...
            return Datatables::of($products)
                ...
                ->addColumn('vacc1st', function ($row) {
                    $dob = $row->puppy_dob;
                    $vacc1st = date('Y-m-d', strtotime($dob. ' + 42 days'));
                    return $vacc1st;
                })
                ->addColumn('vacc2nd', function ($row) {
                    $dob = $row->puppy_dob;
                    $vacc2nd = date('Y-m-d', strtotime($vacc2nd. ' + 70 days'));
                    return $vacc2nd;
                })
                ->removeColumn('location')
                ->removeColumn('puppy_name')
        ...
        if (request()->ajax()) {
            ...
            $query = Transaction::join(
                ...
                'c.id'
                    ...
                    ->whereRaw("DATEDIFF( DATE_ADD( transaction_date, INTERVAL IF(transactions.pay_term_type = 'days', transactions.pay_term_number, 30 * transactions.pay_term_number) DAY), '$today') <= 14");
            ...
            return Datatables::of($dues)
                ->editColumn('customer', '@if(!empty($supplier_business_name)) {{$supplier_business_name}}, <br> @endif ' .'<i style="color:{{$custom_field1}};font-size: 18px;" class="fa fas fa-user"></i> {{$customer}}')
                ...
                ->editColumn('next_due_date',function ($row) {
                    ...
                    $nextduedate = date('Y-m-d', strtotime($tDate. ' + 14 days'));
                    return $nextduedate;
                })
                ...
                ->editColumn('action', '@if(auth()->user()->can("sell.create") || auth()->user()->can("direct_sell.access")) <a href="{{action("TransactionPaymentController@addPayment", [$id])}}" class="btn btn-xs btn-success add_payment_modal"><i class="fas fa-money-bill-alt"></i> @lang("purchase.add_payment")</a> <span><a href="{{action("TransactionPaymentController@show", [$id])}}" class="btn btn-xs btn-warning view_payment_modal"><i class="fas fa-money-bill-alt"></i> @lang("View Payment")</a></span>@endif')
                ->removeColumn('supplier_business_name')
