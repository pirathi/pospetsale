...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		@php
			...
			if(!empty($product->brand)){ $breed = $product->brand ;}
		@endphp
		...
		@else
			...
			<p><b>Microchip No : </b>{!! $product->sub_sku !!}</p>
			<p><b>Breed : </b>{!! $breed !!}</p>
			<p><b>Microchip No : </b>{!! $product->	category_id !!}</p>
	...
	@if(!empty($is_direct_sell))
  		...
  		<p class="help-block"><small>Add Additional Informations here</small></p>
	@endif
	</td>
	...
	<td>
		...
		@if(count($sub_units) > 0)
			<br>
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit hide">
                @foreach($sub_units as $key => $value)
